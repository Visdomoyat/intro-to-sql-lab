...
-- select population from countries where region = 'Southern Europe';
-- select name from countries where population = '1000';
-- or
...
-- Write SQL query here
select name, countrycode from cities where countrycode = 'SMR' and name != 'San Marino';
select name, code from countries where code = 'SMR'

-- Clue #5: Oh no, she pulled a switch – there are two cities with very similar names, but in totally different parts of the globe! She's headed to South America as we speak; go find a city whose name is like the one we were headed to, but doesn't end the same. Find out the city, and do another search for what country it's in. Hurry!

-- Write SQL query here
select id, name, countrycode, district, population from cities where name like 'Serra%';
...
-- Write SQL query here
select capital  from countries where region = 'South America';
select c2.name as capital_city from countries c1 join cities c2 on c1.capital = c2.id where c1.region = 'South America' and c1.name = 'Brazil';
...
select name, countrycode, population from cities where countrycode = 'BRA' order by population asc limit 20;